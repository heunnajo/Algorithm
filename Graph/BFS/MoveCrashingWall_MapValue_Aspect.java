import java.util.*;
import java.io.*;
public class MoveCrashingWall_MapValue_Aspect {
	static class mcw{
		int x,y,dCnt,dist;
		...
		while(!q.isEmpty()) {
			...
			for(int d = 0;d<4;d++) {
				...
				if(map[nx][ny] == '1') {//벽인 경우.
					if(cur.dCnt == 0 && !visited[nx][ny][1]) {//다음이동좌표는 드릴 쓸것이기 때문에 중복검사하는 visited의마지막차원은 1에 대해서 검사.
						q.add(new mcw(nx,ny,1,cur.dist+1));
